...
Download `terminal-manager-<version>-macos-arm64.dmg` from the [Releases](https://github.com/alangmartini/unshit-agentic-terminal-manager/releases) page, open it, and drag **Terminal Manager.app** onto **Applications**. The ZIP remains available as an alternative. The bundle is ad-hoc signed rather than notarized, so macOS may refuse the first launch: after trying to open the app, use **System Settings › Privacy & Security › Open Anyway**, then confirm **Open**. The DMG does not remove this Gatekeeper check. The published bundle is an Apple silicon build. For local builds, run `scripts/package-macos.sh` followed by `scripts/create-macos-dmg.sh`; the DMG is written to `dist/`. The bundle contains both `terminal-manager` and its sibling `unshit-ptyd` daemon. macOS 11 (Big Sur) or newer is required; Apple silicon is the currently verified target. The renderer uses wgpu with Metal by default (override with `UNSHIT_RENDER_BACKEND=metal` when troubleshooting).
