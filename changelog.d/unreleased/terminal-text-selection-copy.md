...
- Mouse text selection in the terminal: click-drag to select, double-click to select a word (path-aware), triple-click to select a line, and Shift+click to extend. Selected cells are highlighted on the per-frame render clone without touching the live buffer. Selections are anchored to absolute buffer lines, so they stay pinned to their text as the view scrolls and as output streams (scrolling no longer clears the selection); copying always returns the highlighted text.
