...
fn spawn_check(hooks: Arc<UpdateHooks>, source: CheckSource, config: FeedConfig) {
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || run_check(&hooks, source, &config));
    if spawned.is_err() {
...
/// Fetch the feed and apply the result. Shared by the manual/install worker
/// and the delayed startup check.
fn run_check(hooks: &UpdateHooks, source: CheckSource, config: &FeedConfig) {
...
/// Start the delayed startup check on its own thread. A no-op before
/// [`register_hooks`] runs, which never happens in production: `main` wires
/// hooks up right before calling this.
pub fn start_startup_check() {
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            cleanup_stale_downloads(&hooks.shared);
            std::thread::sleep(startup_delay());
            let config = feed_config();
            let proceed = mutate_with(&hooks.shared, |st| {
            ...
            run_check(&hooks, CheckSource::Startup, &config);
        });
    if spawned.is_err() {
        mutate_with(&on_spawn_error.shared, |st| {
