...
pub fn paint_row(
    ...
    // Content cells, skipping `h_offset` leading characters.
    let mut col = gutter_w;
    for ch in line.chars().skip(h_offset) {
        if col >= cols {
            break;
        }
        grid.set_cell(
...
pub fn repaint_all(grid: &mut CellGrid, buffer: &EditorBuffer, top_line: usize, h_offset: usize) {
    let gutter_w = gutter_width(buffer.line_count());
    for row in 0..grid.rows() {
        grid.reset_line_identity(row);
        paint_row(grid, row, top_line + row, buffer, h_offset, gutter_w);
    }
}
...
pub fn scroll_viewport(
    ...
    if delta >= rows {
        repaint_all(grid, buffer, new_top, h_offset);
        return;
    }
    let gutter_w = gutter_width(buffer.line_count());
    let keep = rows - delta;
    if new_top > old_top {
        ...
        for row in keep..rows {
            grid.reset_line_identity(row);
            paint_row(grid, row, new_top + row, buffer, h_offset, gutter_w);
        }
    } else {
...
mod tests {
    ...
    fn repaint_renders_gutter_and_content() {
        ...
        repaint_all(&mut grid, &buffer, 0, 0);
    ...
    fn scroll_down_keeps_line_ids_of_surviving_rows() {
        ...
        scroll_viewport(&mut grid, &buffer, 0, 3, 0);
    ...
    fn scroll_up_restores_earlier_lines() {
        let buffer = buffer_of(100);
        let mut grid = CellGrid::new(10, 20);
        repaint_all(&mut grid, &buffer, 20, 0);
        scroll_viewport(&mut grid, &buffer, 20, 18, 0);
    ...
    fn jump_larger_than_viewport_repaints() {
        ...
        let mut grid = CellGrid::new(10, 20);
        repaint_all(&mut grid, &buffer, 0, 0);
        scroll_viewport(&mut grid, &buffer, 0, 50, 0);
    ...
    fn h_offset_skips_leading_chars() {
        ...
        repaint_all(&mut grid, &buffer, 0, 2);
