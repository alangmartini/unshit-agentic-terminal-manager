...
use super::{
    apply_report, cpu, platform, telemetry, tree, DaemonListing, PaneResources,
    ProcessAgentObservation, ProcessUsage, ResourceReport, TreeUsage,
};
...
struct SessionRoot {
    pid: u32,
    /// Daemon session id, so a process observation can be rejected when the
    /// pane has since closed or reattached to a different session.
    session_id: u64,
    pane_id: Option<u32>,
}
...
struct Monitor {
    tracker: CpuTracker,
    /// Custom harness rules from the profile's `agent-detection.json`,
    /// re-read on the monitor thread when the file changes.
